...
app.post("/newOrder", async (req, res) => {
    ...
    if (req.body.mode === "BUY") {
        if (holding) {
            let newQty = holding.qty + qtyNum;
            let newAvg = ((holding.qty * holding.avg) + (qtyNum * priceNum)) / newQty;
            holding.qty = newQty;
            holding.avg = newAvg;
            await holding.save();
        } else {
...
app.delete("/deleteOrder/:id", async (req, res) => {
    try {
        ...
        if (order) {
            let holding = await HoldingsModel.findOne({ name: order.name });
            let qtyNum = Number(order.qty);
            let priceNum = Number(order.price);
